...
import { Loader } from "../../Components/Header/Loader.component";
                ...
                >
                  ...
                  <VStack spacing={0} w="100%" alignItems="flex-start">
                    {list.map((e: any, i: number) => (
                      <Box key={i} w="100%" onClick={onClose}>
                        <Link
...
export const SearchInput = ({ onPick }: { onPick: () => void }) => {
  ...
  const renderState = () => {
    switch (state) {
      case "t":
        return (
          <Box
            filter={"white" === "white" ? "grayscale(1)" : "invert(1)"}
          ></Box>
        );
      case "l":
        return <Loader />;
